...
c      Refinement flag construction calls hierarchy and buffering routines
c      that share AMR work arrays.  Run this comparatively small regridding
c      phase in deterministic grid order; patch advancement remains parallel.
       do  jg = 1, numgrids(lcheck)
c          mptr = listgrids(jg)
          levSt  = listStart(lcheck)
          mptr   = listOfGrids(levSt+jg-1)
