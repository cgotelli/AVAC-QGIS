...
recursive subroutine filrecur(level,nvar,valbig,aux,naux,t,mx,my, &
    ...
    use geoclaw_module, only: sea_level, dry_tolerance, conserve_depth_amr
    ...
    real(kind=8) :: min_fine_value, slope_scale
    real(kind=8) :: max_offset_x, max_offset_y
    ...
    if (.not.set) then
        ...
        ! Construct the coarse quantity that will be interpolated.  Standard
        ! GeoClaw interpolates eta so a lake at rest remains at rest.  AVAC
        ! instead interpolates flow depth: eta interpolation over steep,
        ! sub-cell topography creates granular mass whenever a fine patch is
        ! born at a wet/dry front.
        ...
        do j_coarse = 1, my_coarse
            do i_coarse = 1, mx_coarse
                ...
                if (conserve_depth_amr) then
                    eta_coarse(i_coarse,j_coarse) = max(h, 0.d0)
        ...
        ! A two-dimensional limited reconstruction can still become negative
        ! at a child-cell corner because the independently limited x and y
        ! slopes add there.  Scale both slopes together just enough to keep
        ! every child depth nonnegative.  The child offsets are symmetric, so
        ! this retains the parent-cell mean exactly.
        if (conserve_depth_amr) then
            max_offset_x = 0.5d0 * (1.d0 - 1.d0 / ratio_x)
            max_offset_y = 0.5d0 * (1.d0 - 1.d0 / ratio_y)
            do j_coarse = 2, my_coarse - 1
                do i_coarse = 2, mx_coarse - 1
                    h_coarse = eta_coarse(i_coarse,j_coarse)
                    min_fine_value = h_coarse &
                        - max_offset_x * abs(slope(1,i_coarse,j_coarse)) &
                        - max_offset_y * abs(slope(2,i_coarse,j_coarse))
                    if (min_fine_value < 0.d0) then
                        slope_scale = h_coarse / max(h_coarse-min_fine_value, tiny(1.d0))
                        slope(1,i_coarse,j_coarse) = slope_scale * slope(1,i_coarse,j_coarse)
                        slope(2,i_coarse,j_coarse) = slope_scale * slope(2,i_coarse,j_coarse)
                    endif
                enddo
            enddo
        endif
        ...
        do j_fine = 1, my_patch
            ...
            do i_fine = 1, mx_patch
                ...
                if (flaguse(i_fine,j_fine) == 0) then
                    ! Interpolate the selected coarse quantity to the fine
                    ! grid.  In AVAC mode eta_fine is a reconstructed depth;
                    ! otherwise it retains GeoClaw's surface-elevation meaning.
