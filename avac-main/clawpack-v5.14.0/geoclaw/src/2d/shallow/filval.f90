...
subroutine filval(val, mitot, mjtot, dx, dy, level, time,  mic, &
    ...
    use geoclaw_module, only: dry_tolerance, sea_level, conserve_depth_amr
...
!! also might need preicallCopy???
    ...
    do j=2, mjc-1
        do i=2, mic-1
            fineflag(1) = .false.
            ! Standard GeoClaw interpolates free-surface elevation to keep
            ! water at rest over bathymetry.  AVAC's conserved quantity is
            ! granular flow depth, so its newly created fine grids must
            ! interpolate depth directly or topographic sub-cell variation
            ! creates mass at every regrid.
            do ii=-1,1
                if (conserve_depth_amr) then
                    coarseval(2+ii) = max(valc(1,i+ii,j), 0.d0)
                else
            ...
            do jj=-1,1
                if (conserve_depth_amr) then
                    coarseval(2+jj) = max(valc(1,i,j+jj), 0.d0)
                else
            ...
            ! Limit the combined two-dimensional reconstruction so every
            ! child depth remains nonnegative without changing the symmetric
            ! child-cell mean.
            if (conserve_depth_amr) then
                max_offset_x = 0.5d0 * (1.d0 - 1.d0 / dble(refinement_ratio_x))
                max_offset_y = 0.5d0 * (1.d0 - 1.d0 / dble(refinement_ratio_y))
                min_fine_value = valc(1,i,j) &
                    - max_offset_x * abs(slopex) - max_offset_y * abs(slopey)
                if (min_fine_value < 0.d0) then
                    slope_scale = valc(1,i,j) / &
                        max(valc(1,i,j)-min_fine_value, tiny(1.d0))
                    slopex = slope_scale * slopex
                    slopey = slope_scale * slopey
                endif
            endif
