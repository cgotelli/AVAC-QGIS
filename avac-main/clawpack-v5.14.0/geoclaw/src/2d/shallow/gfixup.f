...
c     filval recursively reads the AMR hierarchy through shared interpolation
c     work arrays.  Initializing several newly created patches concurrently
c     can therefore corrupt a small number of cells and later change the AMR
c     layout.  Initialize new patches in grid order; normal timestepping stays
c     parallel in advanc.f.
      do  j = 1, newnumgrids(lcheck)
          mptr = listnewgrids(j)
