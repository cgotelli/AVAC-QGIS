...
subroutine update (level, nvar, naux)
    ...
    use geoclaw_module, only: dry_tolerance, conserve_depth_amr
    use amr_module
    implicit none
...
!     real(kind=8), intent(in) :: level
    ...
    ! Apply fine-grid averages and conservative flux corrections to coarse
    ! cells in deterministic grid order.  Adjacent coarse patches can refer
    ! to registers owned by the same fine patch.
                ...
                do i = iplo-ilo+nghost+1, iphi-ilo+nghost+1
                    do j = jplo-jlo+nghost+1, jphi-jlo+nghost+1
                        ...
                        if (conserve_depth_amr) then
                            ! AVAC evolves depth and momentum as conserved
                            ! cell averages.  Dividing by nwet duplicates the
                            ! wet fine-cell mass into dry subcells whenever a
                            ! refined front is restricted to its parent.
                            hc = hsum / totrat
                            huc = husum / totrat
                            hvc = hvsum / totrat
                        else if (nwet > 0) then
