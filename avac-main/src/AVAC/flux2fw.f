...
c
      logical limit
      integer ilo, ihi
      double precision correction_depth, correction_factor

      correction_depth = 0.d0
      if (coordinate_system.eq.1 .and. maux.ge.3) then
         if (imodel_rh.eq.1) correction_depth =
     &       state_momentum_regularization_depth_rh
         if (imodel_rh.ge.2) correction_depth =
     &       voellmy_state_momentum_regularization_depth_rh
      endif
...
c
c        # Taper only conservative high-order interface corrections.
c        # The identical factor also enters their transverse split below.
         correction_factor = 1.d0
         if (correction_depth.gt.0.d0) then
            correction_factor = shallow_correction_factor(
     &          q1d(1,i-1),q1d(1,i),correction_depth,
     &          aux2(3,i-1),aux2(3,i))
         endif
         do 120 m=1,meqn
            cqxx(m,i) = 0.d0
...
c
  119          continue
            cqxx(m,i) = correction_factor * cqxx(m,i)
