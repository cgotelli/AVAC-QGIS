...
def animation(outdir    = "_output",
    ...
    def save_movie():
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if Path(fname).suffix.lower() == ".html":
                mp4_path = Path(fname).with_suffix(".mp4")
                subprocess.run([
                    "ffmpeg", "-y",
                ...
                with open(fname, "w") as f:
                    f.write(_make_html_video(mp4_b64))
                mp4_path.unlink()
            elif Path(fname).suffix.lower() == ".gif":
                subprocess.run([
                    "ffmpeg", "-y",
                    "-framerate",str(frames_per_s),
                    "-i", str(tmpdir / "lake_%05d.png"),
...
<video controls autoplay loop style="max-width:100%;max-height:100vh">
