! Conservative cell-centred qinit reader shared by the WAVE water solver.
module qinit_module
        ...
        if (qinit_type > 0) then
            do i=1-mbc,mx+mbc
                ...
                do j=1-mbc,my+mbc
                    ...
                    ! The input values are cell averages located at cell
                    ! centres.  Conservatively average their piecewise-
                    ! constant cells over this computational cell.
...
!                        endif
    ...
    function qinit_cell_average(xim,xip,yjm,yjp,component) result(q_average)
        ...
        ! qinit raster onto one computational cell.  Rows are NW to SE.
        implicit none
        ...
        do k = k_first,k_last
            ...
            overlap_y = max(0.d0,min(yjp,source_y_top)-max(yjm,source_y_bottom))
            if (overlap_y <= 0.d0) cycle
            j_north = my_qinit-k+1
            do i = i_first,i_last
                source_x_left = x_domain_low + (i-1)*dx_qinit
                source_x_right = source_x_left + dx_qinit
                overlap_x = max(0.d0,min(xip,source_x_right)-max(xim,source_x_left))
