subroutine src2(meqn,mbc,mx,my,xlower,ylower,dx,dy,q,maux,aux,t,dt)
    ...
    use internal_inflow_module, only: apply_internal_inflow
    ...
    ! Add AVAC mass and momentum at the wet shoreline.  Unlike b4step2,
    ! src2 receives the accepted GeoClaw time step, so the prescribed rates
    ! are integrated exactly once for every successful step.
    call apply_internal_inflow(meqn,mbc,mx,my,xlower,ylower,dx,dy,q,t,dt)
    ...
    if (friction_forcing) then
        do j=1,my
            do i=1,mx
                ...
                if (q(1,i,j) <= max(depth_tolerance,dry_tolerance)) then
                    q(2:3,i,j) = 0.d0
                else
