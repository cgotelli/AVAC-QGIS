...
\hypersetup{
  ...
  pdfauthor={LHE--EPFL},
  pdfsubject={AVAC4QGIS 0.5.10 user interface reference}
}
...
\fancyhead[R]{\small Version 0.5.10}
\fancyfoot[C]{\thepage}
...
\title{\textbf{AVAC4QGIS User Interface Guide}\\[6pt]
       \large Interface reference for AVAC4QGIS 0.5.10}
...
\begin{itemize}
  \item \control{DEM raster layer} selects the fixed basal topography. The
        ...
        subdivide the chosen computational cell size.
...
The code replaces nonfinite corrected values with zero, but does not silently
substitute a different scientific correction model.

\begin{figure}[htbp]
\centering
\begin{tikzpicture}[
  node distance=11mm and 12mm,
  block/.style={draw=avacblue,rounded corners,fill=avaclight,align=center,
                minimum width=34mm,minimum height=12mm,font=\small},
  result/.style={draw=avacgray,rounded corners,align=center,
                 minimum width=38mm,minimum height=12mm,font=\small},
  arrow/.style={-{Latex[length=2.5mm]},thick,avacblue}
]
  \node[block] (dem) {Terrain DEM\\$z(x,y)$};
  \node[block,right=of dem] (mask) {Release polygon\\indicator $I_R(x,y)$};
  \node[block,below=of dem] (base) {Base depth $d_0$\\and elevation correction};
  \node[block,below=of mask] (slope) {DEM slope\\and optional slope factor};
  \node[result,below=15mm of $(base)!0.5!(slope)$] (depth)
       {Mobile initial depth\\$h_0=I_R\,d_{corrected}$};
  \node[result,below=of depth] (surface)
       {Preview Initial Snow Surface\\$\eta_0=z+h_0$};
  \draw[arrow] (dem) -- (base);
  \draw[arrow] (dem) -- (slope);
  \draw[arrow] (mask) -- (depth);
  \draw[arrow] (base) -- (depth);
  \draw[arrow] (slope) -- (depth);
  \draw[arrow] (dem) |- (surface);
  \draw[arrow] (depth) -- (surface);
\end{tikzpicture}
\caption{How the visible Release / initial conditions controls produce the
mobile initial depth and its display surface. Outside the release polygon,
$I_R=0$ and therefore $h_0=0$.}
\end{figure}
...
\begin{itemize}
  ...
  \item \control{Land Strickler coefficient} and \control{Water Strickler
        ...
        water-level break. WAVE passes their reciprocals as Manning
        coefficients, so larger Strickler means weaker Manning resistance.
...
This guide is maintained as \pathcode{docs/AVAC\_QGIS\_UI\_REFERENCE.tex}.
Current images are under \pathcode{docs/images/}; they can be regenerated from
the installed QGIS Python environment with
\pathcode{docs/generate\_ui\_screenshots.py}. Compile from the
\pathcode{docs/} directory with \pathcode{latexmk -pdf
