...
\hypersetup{
  ...
  pdfauthor={LHE--EPFL},
  pdfsubject={Tutorial for AVAC4QGIS 1.0.0}
}
...
\begin{notebox}
This tutorial follows the interface distributed with AVAC4QGIS 1.0.0. It
...
\begin{importantbox}
\control{Load input layers into QGIS before selecting them in AVAC4QGIS.}
The plugin's input-layer lists show only compatible layers already loaded in
the current QGIS project. Files stored on disk do not appear in these lists
until you add them to QGIS. Drag the input files into QGIS or use
\control{Layer $>$ Add Layer}, then confirm that they appear in the
\control{Layers} panel before selecting them in the plugin. This applies to
the terrain DEM, release polygons, and any existing lake polygon used by WAVE.
\end{importantbox}
...
First load the terrain DEM and release-polygon layer into the current QGIS
project. \control{If an input is not loaded into QGIS, it will not appear in
...
Load the terrain DEM and any existing lake-polygon layer into the current
QGIS project before selecting them here. \control{Layers that are not loaded
into QGIS do not appear in the input selection lists.} A lake polygon created
with the plugin is loaded and selected automatically, as described below.
