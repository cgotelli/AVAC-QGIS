...
\begin{itemize}
  \item \control{DEM raster layer} selects the fixed basal topography. The
        ...
        subdivide the chosen computational cell size. If the DEM dimensions
        are not divisible by the corresponding number of source cells per
        computational cell, AVAC4QGIS removes the minimum number of source
        rows and columns. It shares the crop as evenly as possible between
        opposite edges and never resamples or extends the DEM. Internally it
        writes one replicated edge-sample halo for
...
\begin{itemize}
  ...
  \item \control{Limiter} selects none, minmod, superbee, MC, or van Leer for
        second-order wave limiting. The default is model-specific: a newly
        selected Coulomb model uses minmod, while Voellmy and cohesive Voellmy
        retain superbee. An explicit selection is remembered separately for
        each model, and saved configurations retain their recorded limiter.
\end{itemize}
